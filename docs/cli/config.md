...
<AccordionGroup>
  <Accordion title="Dry-run behavior">
    ...
    - Builder mode: runs SecretRef resolvability checks for changed refs/providers. A `--ref-*` target outside the supported [SecretRef credential surface](/reference/secretref-credential-surface) also gets the full schema pass.
